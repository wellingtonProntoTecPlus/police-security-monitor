            ...
            {selectedEvent.queueStatus === "maintenance" ? (
              ...
              <div className="flex min-h-0 flex-1 flex-col">
                ...
                <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto p-5 lg:grid-cols-[0.85fr_1.15fr]">
                  <div className="rounded-lg border border-border bg-black/15 p-4"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-bold text-foreground">Providências</h3><Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setShowProceduresModal(true)}>Ver providências</Button></div>{logs.length === 0 ? <p className="mt-3 text-sm text-muted-foreground">Registre acima cada contato e providência tomada durante o atendimento.</p> : <div className="mt-3 space-y-1.5">{logs.slice(-2).map((log, index) => <p key={index} className="line-clamp-1 font-mono text-xs text-muted-foreground">{log}</p>)}</div>}</div>
